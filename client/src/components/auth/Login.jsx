...
export default function Login() {
  ...
  const handleDemoLogin = async () => {
    const demoCredentials = { username: "apollo_name", password: "123456" };
    ...
    <div className="login-page">
      <div className="login-left">
        ...
        <div className="login-features">
          {['AI Powered Automation', 'Multi Specialty Support', 'Dental & Dermatology Ready', 'WhatsApp Automation', 'Appointment Management', 'Prescription & Billing'].map(f => (
        ...
        <div className="login-card">
          ...
          <div style={{ display:'flex', background:'#f8fafc', borderRadius:12, padding:4, marginBottom:24, border:'1px solid #e2e8f0' }}>
            {[['password','🔑 Password'],['otp','📱 OTP Login']].map(([m, l]) => (
              <button key={m} type="button" onClick={() => { setMode(m); setPwErr(''); setOtpErr(''); setOtpStep(1); }}
                style={{ flex:1, padding:'10px 0', border:'none', borderRadius:10, fontSize:14, cursor:'pointer', fontFamily:'var(--font)',
                  background: mode===m ? '#fff' : 'transparent',
                  color: mode===m ? 'var(--primary)' : '#64748b',
                  fontWeight: mode===m ? 600 : 500,
                  boxShadow: mode===m ? '0 2px 6px rgba(0,0,0,0.06)' : 'none',
                  transition:'all 0.2s',
                }}>
                {l}
              </button>
          ...
          {mode === 'password' && (
            <form onSubmit={handlePasswordLogin}>
              {pwErr && <div className="login-error"><span>❌</span> {pwErr}</div>}
              ...
              <div className="form-group">
                ...
                <input type="password" value={pwForm.password} onChange={e => setPwForm(f=>({...f,password:e.target.value}))} autoComplete="current-password" placeholder="••••••••" />
              </div>
          ...
          {mode === 'otp' && (
            <div>
              {otpErr && <div className="login-error"><span>❌</span> {otpErr}</div>}
                ...
                <>
                  <div className="form-group">
                    ...
                    <div style={{ display:'flex', gap:10 }}>
                      <span style={{ padding:'12px 16px', background:'#f8fafc', border:'1.5px solid #e2e8f0', borderRadius:12, fontSize:15, color:'#475569', fontWeight:500, whiteSpace:'nowrap' }}>+91</span>
                    ...
                    <div style={{ fontSize:13, color:'#64748b', marginTop:8 }}>Must match the phone number registered with your account</div>
                  </div>
              ...
              {otpStep === 2 && (
                <>
                  <div style={{ background:'#f0fdf4', border:'1px solid #bbf7d0', borderRadius:12, padding:'16px', marginBottom:20 }}>
                    <div style={{ fontWeight:600, color:'#166534', fontSize:15, marginBottom:6 }}>OTP Generated!</div>
                    <div style={{ fontSize:14, color:'#166534', marginBottom:14 }}>Click the button below to open WhatsApp. Your OTP will be pre-filled — send it to yourself and then enter it here.</div>
                    <a href={waLink} target="_blank" rel="noreferrer"
                      style={{ display:'inline-flex', alignItems:'center', gap:8, padding:'10px 20px', background:'#25D366', color:'#fff', borderRadius:10, textDecoration:'none', fontSize:14, fontWeight:600 }}>
                  ...
                  <div className="form-group">
                    ...
                    <input value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g,'').slice(0,6))} placeholder="6-digit code" autoFocus maxLength={6}
                      style={{ fontSize:24, letterSpacing:12, textAlign:'center', fontFamily:'monospace', fontWeight:600 }}
                      onKeyDown={e => e.key==='Enter' && otp.length===6 && handleVerifyOtp()} />
                    <div style={{ fontSize:13, color:'#64748b', marginTop:8, textAlign:'center' }}>OTP expires in 10 minutes</div>
                  </div>
                  <button className="btn btn-primary login-btn" onClick={handleVerifyOtp} disabled={otpLoading || otp.length < 6}>
                    {otpLoading ? 'Verifying…' : 'Verify & Sign In'}
                  </button>
                  <button style={{ width:'100%', marginTop:12, padding:'8px', border:'none', background:'transparent', color:'#64748b', cursor:'pointer', fontSize:14, fontWeight:500 }} onClick={() => { setOtpStep(1); setOtp(''); setOtpErr(''); }}>← Try different number</button>
                </>
              )}
            </div>
          )}
