...
export default function AppointmentList() {
  ...
  const [appointments, setAppointments] = useState([]); const [total, setTotal] = useState(0);
  const [filter, setFilter] = useState('all'); const [loading, setLoading] = useState(true); const [page, setPage] = useState(1);
  const LIMIT = 30;
  ...
  const fetch_ = async () => {
    ...
    setAppointments(d.appointments||[]); setTotal(d.total||0); setLoading(false);
  };
  useEffect(() => { fetch_(); }, [filter, page]);

  const updateStatus = async (id, status) => {
    await authFetch(`/api/appointments/${id}`, { method:'PUT', body:JSON.stringify({ status }) }); fetch_();
  };
  ...
  return (
    <div className="page">
      <div className="page-header"><div className="page-header-left"><h1>Appointments</h1><p>{total} total</p></div></div>
      <div className="card">
        ...
        <div className="table-container">
          {loading ? <div className="loading"><div className="spinner"></div></div> : (
            <table>
              ...
              <tbody>
                {!appointments.length && <tr><td colSpan={7}><div className="empty-state"><div className="icon">📅</div><p>No appointments found</p></div></td></tr>}
                {appointments.map(a => (
                  <tr key={a.id}>
                    ...
                    <td><span className={`badge badge-${STATUS_BADGE[a.status]||'gray'}`}>{a.status}</span></td>
                    <td>
                      <div style={{ display:'flex', gap:4, flexWrap:'wrap' }}>
                        ...
                        {a.status==='cancelled' && <button className="btn btn-sm btn-secondary" onClick={()=>updateStatus(a.id,'scheduled')}>Restore</button>}
